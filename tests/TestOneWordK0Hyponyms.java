...
public class TestOneWordK0Hyponyms {
    ...
    @Test
    public void testHyponymsSmall() {
        WordnetGraph wng = new WordnetGraph(SMALL_SYNSET_FILE, SMALL_HYPONYM_FILE);
        assertThat(wng.findHyponyms("mutation")).isEqualTo(Set.of("mutation"));
        assertThat(wng.findHyponyms("saltation")).isEqualTo(Set.of("leap", "jump", "saltation"));
        NgordnetQueryHandler studentHandler = AutograderBuddy.getHyponymsHandler(
                WORDS_FILE, TOTAL_COUNTS_FILE, SMALL_SYNSET_FILE, SMALL_HYPONYM_FILE);
        List<String> words = new ArrayList<>();
        words.add("change");
    ...
    @Test
    public void testHyponymsLarge() {
        WordnetGraph wng = new WordnetGraph(LARGE_SYNSET_FILE, LARGE_HYPONYM_FILE);
        assertThat(wng.findHyponyms("Aachen")).isEqualTo(Set.of("Aachen", "Aix-la-Chapelle", "Aken"));
    }
